function make_matrix_for_water_small_ver01(n,save0,save1,save2,save3)
load('NI_first_second_third.mat')
first.obs = n*(first.obs(1:n)./sum(first.obs(1:n)));
...
for i=1:n
    ...
    obs{i,1}=second.obs{i,1};
    numScenarios{i,1}=sum( obs{i,1});
...
for i=1:n
    for j=1:8
        ...
        lb{i,j}=third.lb{i,j};
        ub{i,j}=third.ub{i,j};
        obs{i,j}=third.obs{i,j};
        numScenarios{i,j}=sum(obs{i,j});
