...
def run(inputPHI, alpha, matlab_input_data, matlab_input_data1, matlab_input_data2, matlab_input_data3,savefile):
    ...
    for i in range(lp.first['numScenarios']):
        tmp = np.append(np.zeros_like(philp1[i].lpModel['obj']), [0, 0, 0, q2[i]/4])
        obj = np.append(obj, tmp)

    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            tmp = np.append(np.zeros_like(philp2[i][j].lpModel['obj']), [0, 0, 0, 0])
            obj = np.append(obj, tmp)
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                tmp = np.append(np.zeros_like(philp3[i][j][k].lpModel['obj']), [0, 0])
                obj = np.append(obj, tmp)
    ...
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            tmp = np.append(philp2[i][j].lpModel['lb'], [-cplex.infinity, 0, 0, 0])
            lb = np.append(lb, tmp)
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
    ...
    for i in range(lp.first['numScenarios']):
        tmp = np.append(philp1[i].lpModel['ub'], [cplex.infinity, cplex.infinity, cplex.infinity, cplex.infinity])
        ub = np.append(ub, tmp)
    ...
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                tmp = np.append(philp3[i][j][k].lpModel['ub'], [cplex.infinity, cplex.infinity])
                ub = np.append(ub, tmp)
    ...
    for i in range(lp.first['numScenarios']):
        ...
        col_A2[i] = np.arange((philp.lpModel['A'].shape[1] + 2) + philp1[i].lpModel['A'].shape[1] * i + 4 * i,
                              (philp.lpModel['A'].shape[1] + 2) + philp1[i].lpModel['A'].shape[1] * (i + 1) + 4 * i)
    ...
    col_A3 = np.zeros_like(np.tile(np.arange(philp2[0][0].lpModel['A'].shape[1]),
                                   (lp.first['numScenarios']*lp.second[0]['numScenarios'], 1)))
    for t in range(lp.first['numScenarios']*lp.second[0]['numScenarios']):
        col_A3[t] = np.arange((philp.lpModel['A'].shape[1] + 2 + (philp1[0].lpModel['A'].shape[1]+4)*lp.first['numScenarios']) + philp2[0][0].lpModel['A'].shape[1] * t + 4 * t,
                             (philp.lpModel['A'].shape[1] + 2 + (philp1[0].lpModel['A'].shape[1]+4)*lp.first['numScenarios']) + philp2[0][0].lpModel['A'].shape[1] * (t + 1) + 4 * t)
    ...
    for t in range(lp.first['numScenarios'] * lp.second[0]['numScenarios']* lp.third[0][0]['numScenarios']):
        col_A4[t] = np.arange(
            (philp.lpModel['A'].shape[1] + 2 + (philp1[0].lpModel['A'].shape[1] + 4) * lp.first['numScenarios']+(philp2[0][0].lpModel['A'].shape[1] + 4)* lp.first['numScenarios'] * lp.second[0]['numScenarios']) +
    ...
    for i in range(lp.first['numScenarios']):
        ...
        A_lc2[row_lc2[i], col_A2[i][-1]+1:col_A2[i][-1]+4] = np.array([1,(philp1[i].rho-1) ,-1])
    ...
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            A_lc3[row_lc3[i][j], col_A2[i][-1]+1:col_A2[i][-1]+3] = np.array([-1,2])
            A_lc3[row_lc3[i][j], col_A3[i][j]] = philp2[i][j].lpModel['obj']
            A_lc3[row_lc3[i][j], col_A3[i][j][-1]+1:col_A3[i][j][-1]+4] = np.array([1,(philp2[i][j].rho-1) ,-1])
    ...
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                A_lc4[row_lc4[i][j][k], col_A3[i][j][-1] + 1:col_A3[i][j][-1] + 3] = np.array([-1, 2])
                A_lc4[row_lc4[i][j][k], col_A4[i][j][k]] = philp3[i][j][k].lpModel['obj']
                A_lc4[row_lc4[i][j][k], col_A4[i][j][k][-1] + 1:col_A4[i][j][k][-1] + 2] = np.array([-1])
    ...
    # A_q
    for i in range(lp.first['numScenarios']):
        A_q80_id1  = [int(col_A1[-1])+2,int(col_A2[i][-1])+3]
        A_q80_id2  = [int(col_A2[i][-1])+4, int(col_A2[i][-1]) + 3]
        A_q80_coef = [-1,1]
        q = cplex.SparseTriple(ind1=A_q80_id1, ind2=A_q80_id2, val=A_q80_coef)
    ...
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                A_q4_id1  = [int(col_A3[i][j][-1])+2,int(col_A4[i][j][k][-1])+1]
                A_q4_id2  = [int(col_A4[i][j][k][-1])+2, int(col_A4[i][j][k][-1]) + 1]
                A_q4_coef = [-1,1]
...
if __name__ == "__main__":
    run('mchi2', 0.05, 'water2_0.mat', 'water2_1.mat', 'water2_2.mat', 'water2_3.mat', './result/water2_cqp.txt')
    run('mchi2', 0.05, 'water5_0.mat', 'water5_1.mat', 'water5_2.mat', 'water5_3.mat',
        './result/water5_cqp.txt')
    run('mchi2', 0.05, 'water8_0.mat', 'water8_1.mat', 'water8_2.mat', 'water8_3.mat',
        './result/water8_cqp.txt')

    run('mchi2', 0.05, 'water10_0.mat', 'water10_1.mat', 'water10_2.mat', 'water10_3.mat', './result/water10_cqp.txt')
    run('mchi2', 0.05, 'water20_0.mat', 'water20_1.mat', 'water20_2.mat', 'water20_3.mat',
        './result/water20_cqp.txt')
    run('mchi2', 0.05, 'water40_0.mat', 'water40_1.mat', 'water40_2.mat', 'water40_3.mat',
        './result/water40_cqp.txt')


    run('mchi2', 0.05, 'water80_0.mat', 'water80_1.mat', 'water80_2.mat', 'water80_3.mat','./result/water80_cqp.txt')
    run('mchi2', 0.05, 'water100_0.mat', 'water100_1.mat', 'water100_2.mat', 'water100_3.mat', './result/water100_cqp.txt')
    run('mchi2', 0.05, 'water200_0.mat', 'water200_1.mat', 'water200_2.mat', 'water200_3.mat', './result/water200_cqp.txt')
    # run('mchi2', 0.01, 'NI_first_second_third.mat', 'water80_1.mat', 'water80_2.mat', 'water80_3.mat')
