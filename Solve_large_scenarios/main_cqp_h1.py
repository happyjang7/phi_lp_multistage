...
def run(inputPHI, alpha, matlab_input_data, matlab_input_data1, matlab_input_data2, matlab_input_data3,savefile):
    ...
    for i in range(lp.first['numScenarios']):
        tmp = np.append(np.zeros_like(philp1[i].lpModel['obj']), [0, 0, q2[i],0,0])
        obj = np.append(obj, tmp)

    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            tmp = np.append(np.zeros_like(philp2[i][j].lpModel['obj']), [0, 0, 0, 0, 0])
            obj = np.append(obj, tmp)
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                tmp = np.append(np.zeros_like(philp3[i][j][k].lpModel['obj']), [0, 0, 0])
                obj = np.append(obj, tmp)

    # lb
    lb = np.append(philp.lpModel['lb'], [-cplex.infinity, 0])
    for i in range(lp.first['numScenarios']):
        tmp = np.append(philp1[i].lpModel['lb'], [-cplex.infinity, 0, -cplex.infinity, -cplex.infinity,0])
        lb = np.append(lb, tmp)

    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            tmp = np.append(philp2[i][j].lpModel['lb'], [-cplex.infinity, 0, -cplex.infinity, -cplex.infinity,0])
            lb = np.append(lb, tmp)
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                tmp = np.append(philp3[i][j][k].lpModel['lb'], [-cplex.infinity,-cplex.infinity, 0])
                lb = np.append(lb, tmp)

    # ub
    ub = np.append(philp.lpModel['ub'], [cplex.infinity, cplex.infinity])
    for i in range(lp.first['numScenarios']):
        tmp = np.append(philp1[i].lpModel['ub'], [cplex.infinity, cplex.infinity, cplex.infinity, cplex.infinity, cplex.infinity])
        ub = np.append(ub, tmp)

    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            tmp = np.append(philp2[i][j].lpModel['ub'],
                            [cplex.infinity, cplex.infinity, cplex.infinity, cplex.infinity, cplex.infinity])
            ub = np.append(ub, tmp)
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                tmp = np.append(philp3[i][j][k].lpModel['ub'], [cplex.infinity, cplex.infinity, cplex.infinity])
                ub = np.append(ub, tmp)
    ...
    for i in range(lp.first['numScenarios']):
        sense_linear1 = np.append(sense_linear1, ['E'])
    for i in range(lp.first['numScenarios']):
        sense_linear1 = np.append(sense_linear1, ['E'])
    ...
    for i in range(lp.first['numScenarios']):
        ...
        col_A2[i] = np.arange((philp.lpModel['A'].shape[1] + 2) + philp1[i].lpModel['A'].shape[1] * i + 5 * i,
                              (philp.lpModel['A'].shape[1] + 2) + philp1[i].lpModel['A'].shape[1] * (i + 1) + 5 * i)
    ...
    col_A3 = np.zeros_like(np.tile(np.arange(philp2[0][0].lpModel['A'].shape[1]),
                                   (lp.first['numScenarios']*lp.second[0]['numScenarios'], 1)))
    for t in range(lp.first['numScenarios']*lp.second[0]['numScenarios']):
        col_A3[t] = np.arange((philp.lpModel['A'].shape[1] + 2 + (philp1[0].lpModel['A'].shape[1]+5)*lp.first['numScenarios']) + philp2[0][0].lpModel['A'].shape[1] * t + 5 * t,
                             (philp.lpModel['A'].shape[1] + 2 + (philp1[0].lpModel['A'].shape[1]+5)*lp.first['numScenarios']) + philp2[0][0].lpModel['A'].shape[1] * (t + 1) + 5 * t)
    ...
    for t in range(lp.first['numScenarios'] * lp.second[0]['numScenarios']* lp.third[0][0]['numScenarios']):
        col_A4[t] = np.arange(
            (philp.lpModel['A'].shape[1] + 2 + (philp1[0].lpModel['A'].shape[1] + 5) * lp.first['numScenarios']+(philp2[0][0].lpModel['A'].shape[1] + 5)* lp.first['numScenarios'] * lp.second[0]['numScenarios']) +
            philp3[0][0][0].lpModel['A'].shape[1] * t + 3 * t,
            (philp.lpModel['A'].shape[1] + 2 + (philp1[0].lpModel['A'].shape[1] + 5) * lp.first['numScenarios']+(philp2[0][0].lpModel['A'].shape[1] + 5)* lp.first['numScenarios'] * lp.second[0]['numScenarios']) +
            philp3[0][0][0].lpModel['A'].shape[1] * (t + 1) + 3 * t)
    ...
    # linear_cqp
    A_lc2_1 = lil_matrix((lp.first['numScenarios'], col_num), dtype=np.float)
    row_lc2 = np.arange(lp.first['numScenarios'])
    for i in range(lp.first['numScenarios']):
        A_lc2_1[row_lc2[i], col_A1[-1]+1:col_A1[-1]+3] = np.array([-1,-1])
        A_lc2_1[row_lc2[i], col_A2[i]] = philp1[i].lpModel['obj']
        A_lc2_1[row_lc2[i], col_A2[i][-1]+1:col_A2[i][-1]+3] = np.array([1,(philp1[i].rho-1)])

        q3 = philp1[i].numObsPerScen / philp1[i].numObsTotal
        for j in range(lp.second[i]['numScenarios']):
            A_lc2_1[row_lc2[i], col_A3[i][j][-1]+3] = q3[j]

    A_lc2_2 = lil_matrix((lp.first['numScenarios'], col_num), dtype=np.float)
    row_lc2 = np.arange(lp.first['numScenarios'])
    for i in range(lp.first['numScenarios']):
        A_lc2_2[row_lc2[i], col_A1[-1] + 1:col_A1[-1] + 3] = np.array([-1, -1])
        A_lc2_2[row_lc2[i], col_A2[i]] = philp1[i].lpModel['obj']
        A_lc2_2[row_lc2[i], col_A2[i][-1] + 1:col_A2[i][-1] + 6] = np.array([1, (philp1[i].rho - 1),1,-2,0])

        q3 = philp1[i].numObsPerScen / philp1[i].numObsTotal
        for j in range(lp.second[i]['numScenarios']):
            A_lc2_2[row_lc2[i], col_A3[i][j][-1] + 3] = q3[j]

    A_lc2_3 = lil_matrix((lp.first['numScenarios'], col_num), dtype=np.float)
    row_lc2 = np.arange(lp.first['numScenarios'])
    for i in range(lp.first['numScenarios']):
        A_lc2_3[row_lc2[i], col_A1[-1] + 1:col_A1[-1] + 3] = np.array([-1, -1])
        A_lc2_3[row_lc2[i], col_A2[i]] = philp1[i].lpModel['obj']
        A_lc2_3[row_lc2[i], col_A2[i][-1] + 1:col_A2[i][-1] + 6] = np.array([1, (philp1[i].rho - 1), -1,0,2])

        q3 = philp1[i].numObsPerScen / philp1[i].numObsTotal
        for j in range(lp.second[i]['numScenarios']):
            A_lc2_3[row_lc2[i], col_A3[i][j][-1] + 3] = q3[j]
    A_lc2 = vstack([vstack([A_lc2_1, A_lc2_2]), A_lc2_3])



    A_lc3_1 = lil_matrix((lp.first['numScenarios']*lp.second[0]['numScenarios'], col_num), dtype=np.float)
    ...
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            A_lc3_1[row_lc3[i][j], col_A2[i][-1]+1:col_A2[i][-1]+3] = np.array([-1,-1])
            A_lc3_1[row_lc3[i][j], col_A3[i][j]] = philp2[i][j].lpModel['obj']
            A_lc3_1[row_lc3[i][j], col_A3[i][j][-1]+1:col_A3[i][j][-1]+3] = np.array([1,(philp2[i][j].rho-1)])

            q4 = philp2[i][j].numObsPerScen / philp2[i][j].numObsTotal
            for k in range(lp.third[i][j]['numScenarios']):
                A_lc3_1[row_lc3[i][j], col_A4[i][j][k][-1] + 1] = q4[k]

    A_lc3_2 = lil_matrix((lp.first['numScenarios'] * lp.second[0]['numScenarios'], col_num), dtype=np.float)
    row_lc3 = np.zeros_like(np.tile(1, (lp.first['numScenarios'] * lp.second[0]['numScenarios'], 1)))
    for t in range(lp.first['numScenarios'] * lp.second[0]['numScenarios']):
        row_lc3[t] = np.arange(t, t + 1)
    row_lc3 = row_lc3.reshape((lp.first['numScenarios'], lp.second[0]['numScenarios'], row_lc3[0].size))
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            A_lc3_2[row_lc3[i][j], col_A2[i][-1] + 1:col_A2[i][-1] + 3] = np.array([-1, -1])
            A_lc3_2[row_lc3[i][j], col_A3[i][j]] = philp2[i][j].lpModel['obj']
            A_lc3_2[row_lc3[i][j], col_A3[i][j][-1] + 1:col_A3[i][j][-1] + 6] = np.array([1, (philp2[i][j].rho - 1),1,-2,0])

            q4 = philp2[i][j].numObsPerScen / philp2[i][j].numObsTotal
            for k in range(lp.third[i][j]['numScenarios']):
                A_lc3_2[row_lc3[i][j], col_A4[i][j][k][-1] + 1] = q4[k]

    A_lc3_3 = lil_matrix((lp.first['numScenarios'] * lp.second[0]['numScenarios'], col_num), dtype=np.float)
    row_lc3 = np.zeros_like(np.tile(1, (lp.first['numScenarios'] * lp.second[0]['numScenarios'], 1)))
    for t in range(lp.first['numScenarios'] * lp.second[0]['numScenarios']):
        row_lc3[t] = np.arange(t, t + 1)
    row_lc3 = row_lc3.reshape((lp.first['numScenarios'], lp.second[0]['numScenarios'], row_lc3[0].size))
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            A_lc3_3[row_lc3[i][j], col_A2[i][-1] + 1:col_A2[i][-1] + 3] = np.array([-1, -1])
            A_lc3_3[row_lc3[i][j], col_A3[i][j]] = philp2[i][j].lpModel['obj']
            A_lc3_3[row_lc3[i][j], col_A3[i][j][-1] + 1:col_A3[i][j][-1] + 6] = np.array([1, (philp2[i][j].rho - 1), -1, 0,2])

            q4 = philp2[i][j].numObsPerScen / philp2[i][j].numObsTotal
            for k in range(lp.third[i][j]['numScenarios']):
                A_lc3_3[row_lc3[i][j], col_A4[i][j][k][-1] + 1] = q4[k]
    A_lc3 = vstack([vstack([A_lc3_1, A_lc3_2]), A_lc3_3])





    A_lc4_1 = lil_matrix((lp.first['numScenarios'] * lp.second[0]['numScenarios']*lp.third[0][0]['numScenarios'], col_num), dtype=np.float)
    ...
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                A_lc4_1[row_lc4[i][j][k], col_A3[i][j][-1] + 1:col_A3[i][j][-1] + 3] = np.array([-1, -1])
                A_lc4_1[row_lc4[i][j][k], col_A4[i][j][k]] = philp3[i][j][k].lpModel['obj']


    A_lc4_2 = lil_matrix(
        (lp.first['numScenarios'] * lp.second[0]['numScenarios'] * lp.third[0][0]['numScenarios'], col_num),
        dtype=np.float)
    row_lc4 = np.zeros_like(
        np.tile(1, (lp.first['numScenarios'] * lp.second[0]['numScenarios'] * lp.third[0][0]['numScenarios'], 1)))
    for t in range(lp.first['numScenarios'] * lp.second[0]['numScenarios'] * lp.third[0][0]['numScenarios']):
        row_lc4[t] = np.arange(t, t + 1)
    row_lc4 = row_lc4.reshape(
        (lp.first['numScenarios'], lp.second[0]['numScenarios'], lp.third[0][0]['numScenarios'], row_lc4[0].size))
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                A_lc4_2[row_lc4[i][j][k], col_A3[i][j][-1] + 1:col_A3[i][j][-1] + 3] = np.array([-1, -1])
                A_lc4_2[row_lc4[i][j][k], col_A4[i][j][k]] = philp3[i][j][k].lpModel['obj']
                A_lc4_2[row_lc4[i][j][k], col_A4[i][j][k][-1] + 1:col_A4[i][j][k][-1] + 4] = np.array([1,-2,0])
    ...
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                A_lc4_3[row_lc4[i][j][k], col_A3[i][j][-1] + 1:col_A3[i][j][-1] + 3] = np.array([-1, -1])
                A_lc4_3[row_lc4[i][j][k], col_A4[i][j][k]] = philp3[i][j][k].lpModel['obj']
                A_lc4_3[row_lc4[i][j][k], col_A4[i][j][k][-1] + 1:col_A4[i][j][k][-1] + 4] = np.array([-1, 0,2])
    A_lc4 = vstack([vstack([A_lc4_1, A_lc4_2]), A_lc4_3])
    ...
    # A_q
    for i in range(lp.first['numScenarios']):
        A_q2_id1  = [int(col_A1[-1])+2, int(col_A2[i][-1])+4, int(col_A2[i][-1])+5]
        A_q2_id2  = [int(col_A1[-1])+2, int(col_A2[i][-1])+4, int(col_A2[i][-1])+5]
        A_q2_coef = [1,1,-1]
    ...
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            A_q3_id1  = [int(col_A2[i][-1])+2, int(col_A3[i][j][-1])+4, int(col_A3[i][j][-1])+5]
            A_q3_id2  = [int(col_A2[i][-1])+2, int(col_A3[i][j][-1])+4, int(col_A3[i][j][-1])+5]
            A_q3_coef = [1,1,-1]
    ...
    for i in range(lp.first['numScenarios']):
        for j in range(lp.second[i]['numScenarios']):
            for k in range(lp.third[i][j]['numScenarios']):
                A_q4_id1  = [int(col_A3[i][j][-1])+2, int(col_A4[i][j][k][-1])+2, int(col_A4[i][j][k][-1])+3]
                A_q4_id2  = [int(col_A3[i][j][-1])+2, int(col_A4[i][j][k][-1])+2, int(col_A4[i][j][k][-1])+3]
                A_q4_coef = [1,1,-1]
...
if __name__ == "__main__":
    for n in [2,8, 24, 40, 56, 72, 88, 104, 120, 136, 152, 168, 184, 200, 216, 232, 248, 264, 280, 296, 312, 328, 344,
              360, 376]:
        run('hellinger', 0.05, 'water' + str(n) + '_0.mat', 'water' + str(n) + '_1.mat', 'water' + str(n) + '_2.mat', './result/hellinger_water' + str(n) + '_cqp.txt')
    run('hellinger', 0.05, 'NI_first_second_third.mat', 'NI_fourth1.mat', 'NI_fourth2.mat', 'NI_fourth3.mat',
        './result/hellinger_water' + str(384) + '_cqp.txt')